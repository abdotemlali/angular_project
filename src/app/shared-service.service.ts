...
export class SharedService {
  ...
  private commentairesSource = new BehaviorSubject<Commentaire[]>([]);
  commentaires$ = this.commentairesSource.asObservable();

  private detail = new BehaviorSubject<number>(0);
  ...
  getCommentairesForProduit(produitId: number): Commentaire[] {
    return this.commentairesSource.value.filter(comment => comment.produitId === produitId);
  }

  // Ajouter un nouveau commentaire
  addCommentaire(commentaire: Commentaire) {
    const currentCommentaires = this.commentairesSource.value;
    this.commentairesSource.next([...currentCommentaires, commentaire]);
  }
